  ...
  function fetchLatestPosts() {
    fetch(`http://localhost:3000/posts/latest?limit=3`)
      ...
      .catch((error) => {
        ...
        const lastPostsContainer = document.getElementById(
  ...
  function renderPost(post) {
    ...
    if (postCategory) {
      const primaryCategory =
        post.formatCategory || post.contentCategory || "Category";
    ...
    if (post.editDates && post.editDates.length > 0) {
      const lastEditDate = new Date(
        post.editDates[post.editDates.length - 1]
      ).toLocaleDateString();
      editDateHtml = `<span class="edit-indicator">(Güncellenme: ${lastEditDate})</span>`;
    }
    const formattedContentCategoryName =
      post.formatCategory.charAt(0).toUpperCase() +
      post.formatCategory.slice(1);
    const formattedFormatCategoryName =
      post.contentCategory.charAt(0).toUpperCase() +
      post.contentCategory.slice(1);
    ...
    if (post.content && Array.isArray(post.content)) {
      ...
      for (let i = 0; i < post.content.length; i++) {
        ...
        if (block.type === "paragraph") {
          ...
          postHtml += `<blockquote><p>${content}</p>${
          ...
          if (
            ...
            imageUrl.includes("content-images/")
          ) {
                  ...
                  <div class="tags">
                    ...
                    <div class="tag-list">
                      ${post.tags
                        .map(
                          (tag) =>
                            `<a href="tag.html?tag=${encodeURIComponent(
                              tag
                            )}">${tag}</a>`
                        )
                        .join("")}
      ...
      postsHtml += `
          <div class="col-lg-4 col-md-6">
            <div class="single-post-wrap">
              <div class="thumb">
                <img src="${thumbnailUrl}" alt="${post.title}">
                <a class="tag-base tag-${
                  post.formatCategory === "anime" ? "blue" : "purple"
                }" href="category.html?slug=${post.formatCategory}">${
        post.formatCategory
      }</a>
              </div>
              <div class="details">
                ...
                <h6 class="title mt-2"><a href="post.html?id=${post._id}">${
        post.title
      }</a></h6>
              </div>
            </div>
