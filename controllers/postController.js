...
exports.createPost = asyncHandler(async (req, res) => {
  ...
  if (saved.status) {
    try {
      ...
      if (saved.thumbnail) {
        try {
          thumbnailUrl = cloudinary.url(saved.thumbnail, {
            resource_type: "image",
            width: 600,
            height: 400,
            crop: "fill",
          ...
          thumbnailUrl = saved.thumbnail; // Use original if transformation fails
        }
      }
      ...
      if (saved.content && saved.content.length > 0) {
        ...
        const textBlock = saved.content.find(block => {
      ...
      if (!excerpt) {
        excerpt = `${saved.title} hakkında yeni bir yazı yayınladık. Detaylar için yazıyı okuyun!`;
      }
...
exports.updatePost = asyncHandler(async (req, res) => {
  ...
  // Update post data
  const wasPublished = post.status; // Store old status
